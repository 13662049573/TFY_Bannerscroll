...
@implementation TFY_BannerImageView
...
- (void)setAnimatedImage:(TFY_BannerAnimatedImage *)animatedImage
{
...
@interface TFY_BannerAnimatedImage ()
...
typedef NS_ENUM(NSUInteger, AnimatedImageDataSizeCategory) {
    AnimatedImageDataSizeCategoryAll = 10,       // 所有帧都永久保存在内存中(对CPU好一点)
    AnimatedImageDataSizeCategoryDefault = 75,   // 内存中默认大小的帧缓存(通常是实时性能和保持低内存配置)
    AnimatedImageDataSizeCategoryOnDemand = 250, // 每次只在内存中保存一帧(内存更容易，性能更慢)
    AnimatedImageDataSizeCategoryUnsupported     // 即使是太大的一帧，电脑也会拒绝。
};

typedef NS_ENUM(NSUInteger, AnimatedImageFrameCacheSize) {
    AnimatedImageFrameCacheSizeNoLimit = 0,                // 0表示没有特定的限制
    AnimatedImageFrameCacheSizeLowMemory = 1,              // 最小帧缓存大小;这将产生按需帧。
    AnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // 如果我们生产帧的速度比消耗帧的速度快，那么提前一帧就可以实现无口吃的回放。
    AnimatedImageFrameCacheSizeDefault = 5                 // 建立一个舒适的缓冲窗口来处理CPU的打嗝等。
};


@implementation TFY_BannerAnimatedImage
...
+ (void)initialize
{
    if (self == [TFY_BannerAnimatedImage class]) {
...
- (instancetype)init
{
    TFY_BannerAnimatedImage *animatedImage = [self initWithAnimatedGIFData:NSData.new];
...
{
    TFY_BannerAnimatedImage *animatedImage = [[TFY_BannerAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    TFY_BannerAnimatedImage * __weak weakSelf = self;
...
@implementation TFY_BannerWeakProxy

#pragma mark Life Cycle
+ (instancetype)weakProxyForObject:(id)targetObject {
    TFY_BannerWeakProxy *weakProxy = [TFY_BannerWeakProxy alloc];
