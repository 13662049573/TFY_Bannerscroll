//
//  TFY_PersentInteractiveTransition.m
//  TFY_PlayerTools
//
...
@interface TFY_PersentInteractiveTransition ()<UIGestureRecognizerDelegate>
...
@implementation TFY_PersentInteractiveTransition
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    DisablePortraitGestureTypes type = DisablePortraitGestureTypesNone;
    if (gestureRecognizer == self.tapGesture) type = DisablePortraitGestureTypesTap;
    else if (gestureRecognizer == self.panGesture) type = DisablePortraitGestureTypesPan;
    else return NO;

    switch (type) {
        case DisablePortraitGestureTypesTap: {
            if (self.disablePortraitGestureTypes & DisablePortraitGestureTypesTap) {
                return NO;
            }
        }
            break;
        case DisablePortraitGestureTypesPan: {
            if (self.disablePortraitGestureTypes & DisablePortraitGestureTypesPan) {
                return NO;
            }
        }
            break;
...
- (void)interPercentFinish {
    ...
    [self.delagate tfy_orientationWillChange:NO];
    [UIView animateWithDuration:0.3f animations:^{
        ...
        [self.bgView removeFromSuperview];
        fromVC.view.backgroundColor = [UIColor blackColor];
        [self.delagate tfy_orientationDidChanged:NO];
        [transitionContext completeTransition:![transitionContext transitionWasCancelled]];
    }];
}
...
- (void)setInteration:(BOOL)interation {
    ...
    if ([self.delagate respondsToSelector:@selector(tfy_interationState:)]) {
        [self.delagate tfy_interationState:interation];
    }
}
